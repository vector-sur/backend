...
pub async fn increment_user_stats(pool: &MySqlPool) -> Result<(), sqlx::Error> {
    sqlx::query!(
        "UPDATE stats SET total_accounts = total_accounts + 1, active_accounts = active_accounts \
         + 1 WHERE id = 1"
    )
    .execute(pool)
    .await?;
...
pub async fn deactivate_user_stats(pool: &MySqlPool) -> Result<(), sqlx::Error> {
    sqlx::query!(
        "UPDATE stats SET active_accounts = active_accounts - 1, inactive_accounts = \
         inactive_accounts + 1 WHERE id = 1"
    )
    .execute(pool)
    .await?;
